...
from ollama_utils import ollama_chat_completion
from role.configs import ChatGPTConfig
from role.messages import ChatMessage, MessageType, SystemMessage
from role.utils import get_model_token_limit, num_tokens_from_messages
...
class ChatAgent:
    ...
    def step(
        ...
        if num_tokens < self.model_token_limit:
            ...
            try:
                # Convert openai_messages format to format suitable for ollama
                ollama_messages = []
                system_message = None
                ...
                content, token = ollama_chat_completion(ollama_messages, system_message)
                
                # Simulate OpenAI response format to maintain compatibility
                response = {
                    "id": "ollama_response",
                    "usage": {"total_tokens": num_tokens, "cord_token": token},
                    "choices": [{
                        "message": {
                            "content": content,
                            "role": "assistant"
