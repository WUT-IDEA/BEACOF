...
from simulator_ablation_belief import InteractiveGameEngine
...
async def run(scenario: str = "medqa", retry_cases: list[str | int] = []) -> None:
    ...
    if scenario == "medqa":
        ...
        result_dir = str(
            ...
            / "results"
            / "medical_qa"
            / f"ablation_belief_{config['role_llm']['model_name']}_{timestamp}"
        )
    elif scenario == "court":
        id_key = "case_id"
        ...
        result_dir = str(
            ...
            / "results"
            / "court_debate"
            / f"{config['role_llm']['model_name']}_ablation_belief_{timestamp}"
        )
