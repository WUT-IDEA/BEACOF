...
from simulator_ablation_coop import InteractiveGameEngine
...
async def run(scenario: str = "medqa", retry_cases: list[str | int] = []) -> None:
    ...
    if scenario == "medqa":
        ...
        result_dir = str(
            ...
            / "results"
            / "medical_qa"
            / f"{config['role_llm']['model_name']}_ablation_coop_{timestamp}"
        )
    elif scenario == "court":
        id_key = "case_id"
        ...
        result_dir = str(
            ...
            / "results"
            / "persona_chat"
            / f"ablation_coop_{config['role_llm']['model_name']}_{timestamp}"
        )
    else:
